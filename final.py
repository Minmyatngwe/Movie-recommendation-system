...
def recommending_system(movie):
    ...
    # Iterate through the cosine similarities and append them to list 'a'
    for j, i in enumerate(cos[c]):
        a.append([j, i])
    
    # Sort the list 'a' based on cosine similarities
    b = sorted(a, reverse=True, key=lambda x: x[1])
    
    # Extract top 5 recommended movies from 'b' excluding the input movie itself
    for i in b[0:11]:
...
def get_overview(movie_name):
    id=md_data.loc[md_data.title==movie_name,'movie_id']
